"""Provider real alternativo usando Gemini (Google AI Studio, tier gratuito).

Nao faz parte do roteamento oficial do CLAUDE.md (que e Anthropic/Haiku/
Sonnet) — existe so para testar a qualidade da geracao real sem custo,
antes de decidir habilitar a Fase 7 com a API paga da Anthropic. Reusa os
mesmos prompts de sistema do AnthropicAIProvider (sao independentes de
modelo) para nao duplicar a logica anti-alucinacao/auditoria/mentora.
"""
...
GOOGLE_MODELS = {
    ...
    "audit": "gemini-3.6-flash",
    "mentoria": "gemini-3.6-flash",
}

# Tier gratuito confirmado no painel do usuario: gemini-3.6-flash a 5 RPM.
# 13s de espaco entre chamadas sequenciais fica com folga de seguranca
# abaixo desse limite (nao ha Batch API gratuita equivalente aqui).
...
class GoogleAIProvider(AIProvider):
    ...
    def _generate(self, model: str, system: str, prompt: str, max_output_tokens: int) -> str:
    ...
    def classify_node_types(self, items: list[NodeClassificationInput]) -> list[NodeType]:
        ...
        for item in items:
            ...
            raw = self._generate(GOOGLE_MODELS["classify"], CLASSIFY_NODE_SYSTEM, prompt, 16)
    ...
    def classify_edges(
        ...
        for item in items:
            ...
            raw = self._generate(GOOGLE_MODELS["classify"], CLASSIFY_EDGE_SYSTEM, prompt, 64)
    ...
    def generate_content(self, item: ContentGenerationInput) -> str:
        ...
        return self._generate(GOOGLE_MODELS["generate"], GENERATE_CONTENT_SYSTEM, prompt, 2048)
    ...
    def audit_content(self, text: str, edges: list[EdgeContext]) -> AuditResult:
        ...
        raw = self._generate(
            GOOGLE_MODELS["audit"], AUDIT_SYSTEM,
            f"Texto:\n{text}\n\nArestas permitidas:\n{edges_json}", 128,
        )
    ...
    def mentoria_reply(
        ...
        return self._generate(GOOGLE_MODELS["mentoria"], MENTORA_SYSTEM, prompt, 512)

    def mentoria_veredito(self, node_content: str, history: list[MentoriaTurn]) -> tuple[bool, str]:
        transcript = "\n".join(f"{t.role}: {t.text}" for t in history)
        raw = self._generate(
            GOOGLE_MODELS["mentoria"], VEREDITO_SYSTEM,
